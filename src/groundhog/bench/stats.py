...
class PairedStats:
    ...
    ``mean_delta``/``sd_delta`` cover only the complete pairs (both sides
    present) — ``n_a``, ``n_b``, ``only_a``, and ``only_b`` say how many
    values each side actually produced, so a reader can see when the delta
    is computed over a survivor subset that disagrees with the win counts.
    """
    metric: str
...
def paired_stats(metric: str, a_values: Sequence, b_values: Sequence,
    ...
    present; ``mean_delta``/``sd_delta`` summarize those, and the one-sided
    pairs are counted explicitly (``only_a``/``only_b``) instead of being
    silently dropped.
    """
    ...
    return PairedStats(
        ...
        n_a=sum(1 for v in a_values if v is not None),
        n_b=sum(1 for v in b_values if v is not None),
        only_a=only_a, only_b=only_b,
        wins_a=wins_a, wins_b=wins_b, ties=ties,
        mean_a=mean_a, mean_b=mean_b, spread_a=spread_a, spread_b=spread_b,
        mean_delta=mean_delta, sd_delta=sd_delta, deltas=deltas,
    )
